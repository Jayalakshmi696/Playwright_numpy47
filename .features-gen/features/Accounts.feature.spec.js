...
test.describe('Accounts module functionality of suite8demo application', () => {
  ...
  test('Verify Accounts module navigation', { tag: ['@AccountsModule', '@Accounts1'] }, async ({ When, Then, accountsPage, loggedInPage }) => { 
    await When('the user clicks on Accounts module from left navigation', null, { accountsPage }); 
    await Then('the user should be navigated to Accounts Dashboard page and should see the list in dropdown', {"dataTable":{"rows":[{"cells":[{"value":"Create Account"}]},{"cells":[{"value":"Import Accounts"}]},{"cells":[{"value":"View Accounts"}]}]}}, { accountsPage, loggedInPage }); 
  });
  ...
  test('Verify Create Account page with valid data', { tag: ['@AccountsModule'] }, async ({ Given, When, Then, accountsPage }) => { 
    await Given('User is on Create Account page', null, { accountsPage }); 
